	...
	if ($actualizoFlag || $insertoFlag)
	{
		$sqlSelect = "SELECT caux.SGD_CAUX_CODIGO, 
					...
					FROM SGD_CAUX_CAUSALES caux
                                        left join SGD_DCAU_CAUSAL dcau on  dcau.SGD_DCAU_CODIGO = caux.SGD_DCAU_CODIGO
					left join SGD_CAU_CAUSAL cau on cau.SGD_CAU_CODIGO = caux.SGD_DCAU_CODIGO
					WHERE caux.RADI_NUME_RADI = '$verradicado'";
...
if(!$rsDetalleCau->EOF)
{
	if (empty($causal_new))
	{
		...
		$causal_new = $rsDetalleCau->fields["SGD_DCAU_CODIGO"];
}	}
