...
// En este proceso se utilizan las variabels $item, $textElements, $newText que son temporales para esta operacion.
	...
	while (!$rsMarcar->EOF)
	    ...
	    $nurad = $anex_radi_nume;
  	    ...
  	    if ($nurad)$radicado = $nurad;
            $pCodDep = $dpto;
            $pCodMun = $muni;
            $nombre_us    = $otro . "-".substr($nombre . " " . $papel . " " . $sapel,0 ,50);
	...
	*/
	    ...
	    if ($error!="no")
            ...
            $odt = new OpenDocText();
            $observa= "_";
            $campos_tabla = array("$verrad_sal","$padre","$nombre_us","$direccion_us","$destino","$departamento","$observa");
            $btt->campos_tabla = $campos_tabla;
            $btt->tabla_Cuerpo();
            error_reporting(7);
	...
	$pdf->WriteHTML( iconv($odt->codificacion($encabezado), 'ISO-8859-1',$encabezado). iconv($odt->codificacion($html), 'ISO-8859-1',$html). $fin);
