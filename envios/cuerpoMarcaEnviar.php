...
//variable con elementos de sesi�n
        ...
        <script>
            ...
            function listadoEntrega(){
                var eligio = false;
                    for(i=1;i<document.formEnviar.elements.length;i++){
                        if(document.formEnviar.elements[i].checked){
                            eligio = true;
                        }
                    }
                    if(!eligio){
                        alert("Debe elegir al menos un radicado para imprimir el listado");
                        return false;
                    }
                    document.formEnviar.action = "cuerpoListaImpresos.php?<?php echo session_name(). "=" . session_id() ."&krd=$krd&fechah=$fechah&dep_sel=$dep_sel&estado_sal=$estado_sal&usua_perm_impresion=$usua_perm_impresion&estado_sal_max=$estado_sal_max&generar_listado=SI&listadoRadicado=SI" ?>";
...
if ($usua_perm_firma == 2 || $usua_perm_firma == 3) {
    ?>
        ...
        <form name='formEnviar'  method="post" action=<?= $pagina_sig ?>?<?= session_name() . "=" . session_id() . "&krd=$krd&fechah=$fechah&dep_sel=$dep_sel&estado_sal=$estado_sal&usua_perm_impresion=$usua_perm_impresion&estado_sal_max=$estado_sal_max" ?> >
            <table width="98%" align="center" cellspacing="0" cellpadding="0">
                <tr>
                    <td class="grisCCCCCC">
