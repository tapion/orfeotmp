...
if (isset($_POST['btn_enviar']))
{
    ...
    {   define('ADODB_ASSOC_CASE', 1);
		...
		while (!$rs->EOF)
		{
			...
			$tmp = str_replace('reinicio.php','bodega',$_SERVER['SCRIPT_FILENAME']);
            $tmp .= "/$ano/".$dep."/docs";
			$ok = rmkdir($tmp);
			if ($ok)
	...
	{	define('ADODB_ASSOC_CASE', 1);
        ...
        $sql = "select * from all_sequences where sequence_owner='".strtoupper($usuario)."' and sequence_name like 'SECR_TP%'";
        $rs = $conn->Execute($sql);
