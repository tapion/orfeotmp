...
?>
<title>HISTORICO ARCHIVO DE GESTION</title>
<? }?>
...
<table width="1024" border="0" cellpadding="0" cellspacing="0" align="center">
  <tr bgcolor="#006699">
    <td class="titulos4" colspan="6" ><center>
    <?if($tip!=2){?>
    HISTORICO ARCHIVO CENTRAL 
    <?}
    else{?>
    HISTORICO ARCHIVO DE GESTION
    <?}?>
    </center></td>
	 </tr>
</table>
...
<td class="listado2" width="200"><?=$rs->fields["HIST_OBSERVA"]?></td>
  ...
  // Finaliza Historicos
	...
	<? if($tip!=2){?>
	<a href='busqueda_central.php?<?=session_name()?>=<?=trim(session_id())?>krd=<?=$krd?>&Buscar'>
	<?}
	else{?>
	<a href='busqueda_Fondo_Gestion.php?<?=session_name()?>=<?=trim(session_id())?>krd=<?=$krd?>&Buscar'>
	<?}?>
	<input name='Regresar' align="middle" type="button" class="botones" id="envia22" value="Regresar" ></td>
	</tr>
</table>
