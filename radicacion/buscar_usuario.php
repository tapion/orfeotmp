...
{	
    ...
    if(document.getElementById('codigo').value=="" && obj.value=="MODIFICAR" && document.getElementById('tagregar').value==0)
    {
       alert("El usuario debe estar creado para poder modificarlo!");
       return false;
    }
    else if(document.getElementById('codigo').value=="" && obj.value=="MODIFICAR" && document.getElementById('tagregar').value==2)
    {
       alert("La Empresa debe estar creada para poder modificarla!");
       return false;
    }
    band=false;
    msg="Debe corregir lo siguiente:\n";
	...
	{	msg+="Seleccione la geograf\xEDa completa del destinatario\n";
		band = true;
	}
	if (document.formu1.direccion_nus1.value=='')
	{	msg+="Debe colocar una Direcci\xF3n.\n";
		band = true;
	}
	if (document.formu1.nombre_nus1.value=='')
	{	msg +="Debe colocar un nombre.\n";
		band= true;
	}
        if (document.formu1.no_documento1.value.length>13)
	{	msg += "!Excedi\xF3 el numero de caracteres permitido (13) para el Documento!\n";
		band= true;
	}
        if (document.formu1.nombre_nus1.value.length>148)
	{	msg+="!Excedi\xF3 el numero de caracteres permitido (150) para el Nombre!\n";
		band=true;
	}

        if (document.formu1.tagregar.value==0)
	{
                if (document.formu1.prim_apell_nus1.value.length>49)
                {	msg+="!Excedi\xF3 el numero de caracteres permitido (50) para el Primer Apellido!\n";
                        band=true;
                }
                if (document.formu1.seg_apell_nus1.value.length>49)
                {	msg+="!Excedi\xF3 el numero de caracteres permitido (50) para el Segundo Apellido!\n";
                        band= true;
                }
        ...
        {
                if (document.formu1.prim_apell_nus1.value.length>49)
                {	msg+="!Excedi\xF3 el numero de caracteres permitido (50) para la Sigla!\n";
                        band= true;
                }
                if (document.formu1.seg_apell_nus1.value.length>148)
                {	msg+="!Excedi\xF3 el numero de caracteres permitido (150) para el Represntante Legal!\n";
                        band = true;
                }
        }

        if (document.formu1.direccion_nus1.value.length>148)
	{	msg+="!Excedi\xF3 el numero de caracteres permitido (150) para la Direcci\xF3n!\n";
		band=true;
	}
        if (document.formu1.telefono_nus1.value.length>49)
	{	msg+="!Excedi\xF3 el numero de caracteres permitido (50) para el Telefono!\n";
		band = true;
	}
        if (document.formu1.mail_nus1.value.length>49)
	{	msg+="!Excedi\xF3 el numero de caracteres permitido (50) para el E-mail!\n";
		band=true;
	}
    if(band)alert(msg);
...
<?php
	...
	?>
    if(opener.document.formulario.otro_us7)//opener.document.formulario.otro_us7.value = document.formu1.seg_apell_us1.value;
	if(opener.document.formulario.otro_us1){opener.document.formulario.otro_us1.focus();opener.focus();}
	 window.close();
}
</script>
</head>
...
{	if($tbusqueda==0)
	{	$array_nombre = split(" ",$nombre_essp);
		...
		if($nombre_essp)
   		{	if($array_nombre[0]) {$where_split = $db->conn->Concat("UPPER($defaultNull (sgd_ciu_nombre $valDefaultNull))","UPPER( $defaultNull (sgd_ciu_apell1 $valDefaultNull) )","UPPER($defaultNull (sgd_ciu_apell2 $valDefaultNull))"). " LIKE '%". strtoupper($array_nombre[0]) ."%' ";}
			if($array_nombre[1]) {$where_split .= " and ". $db->conn->Concat("UPPER($defaultNull (sgd_ciu_nombre $valDefaultNull))","UPPER( $defaultNull (sgd_ciu_apell1 $valDefaultNull) )","UPPER($defaultNull (sgd_ciu_apell2 $valDefaultNull))"). " LIKE '%". strtoupper($array_nombre[1]) ."%' ";}
			if($array_nombre[2]) {$where_split .= " and ". $db->conn->Concat("UPPER($defaultNull (sgd_ciu_nombre $valDefaultNull))","UPPER( $defaultNull (sgd_ciu_apell1 $valDefaultNull) )","UPPER($defaultNull (sgd_ciu_apell2 $valDefaultNull))"). " LIKE '%". strtoupper($array_nombre[2]) ."%' ";}
			if($array_nombre[3]) {$where_split .= " and ". $db->conn->Concat("UPPER($defaultNull (sgd_ciu_nombre $valDefaultNull))","UPPER( $defaultNull (sgd_ciu_apell1 $valDefaultNull) )","UPPER($defaultNull (sgd_ciu_apell2 $valDefaultNull))"). " LIKE '%". strtoupper($array_nombre[3]) ."%' ";}
			$isql .= " where $where_split ";
		}

	   if($no_documento)
	   {	if($nombre_eesp) $isql .= " and "; else 	   $isql .= " where ";
			$isql .= " SGD_CIU_CEDULA='$no_documento' and UPPER($defaultNull (sgd_ciu_nombre$valDefaultNull)) like '%".strtoupper($nombre_essp)."%' ";
	   }
	...
	{	$isql = "select SGD_OEM_NIT AS SGD_CIU_CEDULA,SGD_OEM_OEMPRESA as SGD_CIU_NOMBRE,SGD_OEM_REP_LEGAL as SGD_CIU_APELL2, ".
				...
				"SGD_OEM_SIGLA AS SGD_CIU_APELL1,MUNI_CODI,DPTO_CODI,ID_PAIS,ID_CONT from SGD_OEM_OEMPRESAS	where UPPER($defaultNull (SGD_OEM_OEMPRESA$valDefaultNull)) LIKE '%".strtoupper($nombre_essp)."%' OR UPPER($defaultNull (SGD_OEM_SIGLA$valDefaultNull)) LIKE '%".strtoupper($nombre_essp)."%'";
	...
	{	$isql = "select NIT_DE_LA_EMPRESA AS SGD_CIU_CEDULA,NOMBRE_DE_LA_EMPRESA as SGD_CIU_NOMBRE,SIGLA_DE_LA_EMPRESA as SGD_CIU_APELL1, ".
				...
				"WHERE (UPPER($defaultNull (SIGLA_DE_LA_EMPRESA $valDefaultNull)) LIKE '%".strtoupper($nombre_essp)."%' OR UPPER($defaultNull (NOMBRE_DE_LA_EMPRESA$valDefaultNull)) LIKE '%".strtoupper($nombre_essp)."%') ";
		...
		if($tbusqueda==6)
   		{	$array_nombre = split(" ",$nombre_essp."    ");
			...
			if($nombre_essp)
   			{	if($array_nombre[0]) {$where_split = "  UPPER($defaultNull (USUA_NOMB$valDefaultNull)) LIKE '%". strtoupper($array_nombre[0]) ."%' ";}
				if($array_nombre[1]) {$where_split .= " AND UPPER($defaultNull (USUA_NOMB$valDefaultNull)) LIKE '%". strtoupper($array_nombre[1]) ."%' ";}
				if($array_nombre[2]) {$where_split .= " AND UPPER($defaultNull (USUA_NOMB$valDefaultNull)) LIKE '%". strtoupper($array_nombre[2]) ."%' ";}
				if($array_nombre[3]) {$where_split .= " AND UPPER($defaultNull (USUA_NOMB$valDefaultNull)) LIKE '%". strtoupper($array_nombre[3]) ."%' ";}
...
<tr class='<?=$grilla ?>'> 
	...
	<TD align="center" class="listado5">
		...
		<input type="text" name="cc_documento_us1" value="<?=$cc_documento_us1 ?>" class="ecajasfecha">
	</TD>
	<TD align="center" class="listado5"> <input type="text" name="nombre_us1" value="<?=$nombre_us1?>" class="ecajasfecha" size="15"> </TD>
	<TD align="center" class="listado5"> <input type="text" name="prim_apell_us1" value="<?=$prim_apell_us1 ?>" class="ecajasfecha" size="14"> </TD>
	<TD align="center" class="listado5"> <input type="text" name="seg_apell_us1" value="<?=$seg_apell_us1 ?>" class="ecajasfecha" size="14"> </TD>
	<TD align="center" class="listado5"> <input type="text" name="direccion_us1" value="<?=$direccion_us1 ?>" class="ecajasfecha" size="15"> </TD>
	<TD align="center" class="listado5"> <input type="text" name="telefono_us1" value="<?=$telefono_us1 ?>" class="ecajasfecha" size="10"> </TD>
	<TD align="center" class="listado5"> <input type="text" name="mail_us1" value="<?=$mail_us1 ?>" class="ecajasfecha" size="16"> </TD>
...
<tr align="center" > 
	...
	<TD>  
		<input type="text" name="codigo" id="codigo" class="e_cajas" size="3"  readonly value='<?=$codigo?>' >
