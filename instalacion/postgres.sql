...
BEGIN
    ...
    IF (NumDias>0) THEN
        ...
        WHILE (contador <= NumDias) LOOP
            ...
            fechaFin := cast((fechaFin+ cast((select count(*) from sgd_noh_nohabiles where noh_fecha between fechaFin and fechaFin+cast('1 days' as interval) )||' days' as interval)) as date);
            contador := contador+1;
        END LOOP;
    ELSE
    ...
    IF (diaSemana = 7 or diaSemana = 1) then
...
$BODY$
  LANGUAGE 'plpgsql' VOLATILE;
ALTER FUNCTION sumadiashabiles(date, integer) OWNER TO postgres;
...
BEGIN
    dias:=0;
    fechaAnterior:=to_char(FecInicio,'YYYY/mm/dd');
    if(fechaAnterior<=to_char(fecFin,'YYYY/mm/dd')) then
	    fechaAnterior := dia_sig_ant(fechaAnterior,'Proximo' );
	    WHILE (fechaAnterior<= to_char(fecFin,'YYYY/mm/dd') ) LOOP
		    fechaAnterior := dia_sig_ant(fechaAnterior,'Proximo' );
		    dias := dias-1;
	    END LOOP;
