...
{	case 'mssql':
	...
	case 'postgres':
	 ...
	 $query = "SELECT 
				...
				su.sgd_sbrd_tiemag,
				su.sgd_sbrd_tiemac,
				(CASE WHEN su.SGD_SBRD_DISPFIN = 1 THEN 'C. TOTAL' ELSE CASE WHEN su.SGD_SBRD_DISPFIN = 2 THEN 'ELIMINACION' ELSE CASE WHEN su.SGD_SBRD_DISPFIN = 3 THEN 'M.TECNICO' ELSE  'SELECCION' END END END) AS DISPOSICION,
				su.sgd_sbrd_soporte,
				su.SGD_SBRD_PROCEDI
				FROM SGD_MRD_MATRIRD m,SGD_SRD_SERIESRD s,SGD_SBRD_SUBSERIERD su, SGD_TPR_TPDCUMENTO t, DEPENDENCIA d
				WHERE m.depe_codi = d.depe_codi 
