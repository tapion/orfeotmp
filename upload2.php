...
if ((!$codigo && $userfile1_size==0)||($codigo && $userfile1_size>=return_bytes(ini_get('upload_max_filesize')))) {
    die("<table><tr><td>El tama&ntildeo del archivo no es correcto.</td></tr><tr><td><li>se permiten archivos de ".ini_get('upload_max_filesize')." m&aacute;ximo.</td></tr><tr><td><input type='button' value='cerrar' onclick='opener.regresar();window.close();'></td></tr></table>");
...
{	error_reporting(7);
	...
	else
	{
		if($userfile1) $subir_archivo = " anex_nomb_archivo='1$archivo',anex_tamano = $tamano,anex_tipo=$tipo, "; else {$subir_archivo="";}
	 	$isql = "update anexos set $subir_archivo anex_salida=$anex_salida,sgd_rem_destino=$radicado_rem,sgd_dir_tipo=$radicado_rem,anex_desc='".$_POST['descr']."', SGD_TRAD_CODIGO =".$_POST['tpradic'].", SGD_APLI_CODI = $aplinteg,ANEX_SOLO_LECT='$auxsololect'  where anex_codigo='$codigo'";
	}
//	$db->conn->debug=true;
