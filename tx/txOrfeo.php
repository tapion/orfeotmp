...
<script type="text/javascript" src="<?php echo $ruta_raiz; ?>/js/jquery-1.9.0.min.js"></script>
<script language="javascript">
    ...
    function adminCarrito(input){
        if(input.checked){
            parent.agregaCarrito(input.id);
        }else{
            parent.eliminaCarrito(input.id);
        }        
    }
    function markAll(){
                    ...
                    if (!$agendado) {
                        ...
                        if (($_SESSION['depe_codi_padre'] and $_SESSION['codusuario'] == 1) or $_SESSION['codusuario'] != 1) {
                            if (!empty($permVobo) && $permVobo != 0) {
                                ?>
                                <td width="55" valign="bottom"><a href="#" onmouseout="MM_swapImgRestore()" onclick="seleccionBarra = 14;vistoBueno();" onmouseover="MM_swapImage('Image12','','<?= $ruta_raiz ?>/imagenes/internas/overVobo.gif',1)"><img src="<?= $ruta_raiz ?>/imagenes/internas/vobo.gif" name="Image12" width="55" height="51" border="0" /></a></td>
                                <?
                            }
                        }
                        ?>
                ...
                <tr>
                    ...
                    if (($mostrar_opc_envio == 0) || ($_SESSION['codusuario'] == $radi_usua_actu && $_SESSION['dependencia'] == $radi_depe_actu)) {
                        ...
                        <td width='55%'  align="right" class="titulos2"  >
                            ...
                            $sql = "select $subDependencia, d.depe_codi from DEPENDENCIA d
            inner join usuario u on u.depe_codi=d.depe_codi
            where depe_estado=1 and u.usua_codi=1 $whereReasignar ORDER BY DEPE_NOMB";
                            $rs = $db->query($sql);
                            ...
                            $sql = "select $subDependencia, d.depe_codi from DEPENDENCIA d
            inner join usuario u on u.depe_codi=d.depe_codi
            where depe_estado=1 and u.usua_codi=1 ORDER BY DEPE_NOMB";
