...
if($radicar_documento)
{
	...
	$isql = "update ANEXOS set RADI_NUME_SALIDA=$rad_salida,
	         ...
	         where ANEX_CODIGO='$anexo' AND ANEX_RADI_NUME=$numrad and anex_estado < 4";
	...
	$conexion = $db;
        ...
        $isql = "select anex_codigo, sgd_dir_tipo from anexos where RADI_NUME_SALIDA=$nurad and anex_estado=4";
	...
	$isql = "delete from ANEXOS where RADI_NUME_SALIDA=$nurad
		and sgd_dir_tipo like '7%' and sgd_dir_tipo !=7 and anex_estado<4";
	$rs=$db->query($isql);
	...
	// fIN BORRADO Para reproceso....
    	$isql = "select * from ANEXOS where ANEX_RADI_NUME=$nurad Order by ANEX_NUMERO desc ";
	$rs=$db->query($isql);
	if (!$rs->EOF)
	...
	include_once "./include/query/queryGenarchivo.php";
        ...
        {
            $del_Anex="delete from anexos where radi_nume_salida=$rad_salida and sgd_dir_tipo=".$_GET['radicado_rem']." and anex_nomb_archivo is null";
            $rsdel_Anex=$db->query($del_Anex);
            include "$ruta_raiz/radicacion/grb_direcciones.php";
        }
        $_GET['radicado_rem']==7? $k=1:0;
        $band=0;
        $i=0;
        $sqlUp = "UPDATE anexos
                        set anex_estado=2,
                            ...
                            sgd_deve_codigo=null,
                            sgd_deve_fech=null
                 where radi_nume_salida=$rad_salida and anex_estado < 4";
        $rsUp  = $db->query($sqlUp);
        while(!$rs->EOF)
	{   
            $set="";
            ...
            if($rsVerif->fields['ANEX_CODIGO']===$anexo_new)
            {
                $i++;
                continue;
            }

            if((!isset($enviados) or (!in_array($anexo_new,$enviados,true))) and (!isset($tipoEnviados) or (!in_array($sgd_dir_tipo,$tipoEnviados,true))))
            {
                if($_GET['radicado_rem']!=1 and $band==0 and $sgd_dir_tipo!=1)
                {
                    $sqlVerif="select sgd_renv_codigo from sgd_renv_regenvio where radi_nume_sal=$rad_salida and sgd_dir_tipo=1";
                    $ADODB_COUNTRECS=true;
                    $rsVerif=$db->conn->Execute($sqlVerif);
                    $ADODB_COUNTRECS=false;
                    if($rsVerif && $rsVerif->RecordCount()==0)
                    {
                        $set=" ,sgd_dir_tipo = 1";
                        $sgd_dir_tipo=1;
                        $k=$k-1;
...
$sqlotro="Select SGD_DIR_NOMREMDES, SGD_DIR_DIRECCION 
			From SGD_DIR_DRECCIONES 
			Where radi_nume_radi=$rad_salida 
			And Sgd_Dir_Tipo > 700";
$res=$db->conn->Execute($sqlotro);
$otros_destinatarios = "";
while(!$res->EOF){
    $otros_destinatarios .= $res->fields["SGD_DIR_NOMREMDES"]." ".$res->fields["SGD_DIR_DIRECCION"].", ";
    $res->MoveNext();
}
$otros_destinatarios = trim(trim($otros_destinatarios),",");
...
fputs ($fp,"*OTROS_DESTINATARIOS*=$otros_destinatarios\n");
...
if($ext=="ODT" || $ext=="odt")
{
	...
	foreach ( $contenidoCSV as $line_num => $line )
	{
		...
		{	//Desde la linea 2 hasta el final del archivo de insumo estan los datos de reemplazo
			...
			$cadaLinea[1] = str_replace(">", "'", $cadaLinea[1]);
			//$cadaLinea[1] = iconv($odt->codificacion($nombre_us),'ISO-8859-1',$cadaLinea[1]);
	...
	echo "<B><CENTER><a class='vinculos' href=javascript:abrirArchivo('./bodega/". $linkarchivo_grabar."?time=".time() ."')> Ver Archivo </a><br><br><br>";
	echo "<B><CENTER><a class='vinculos' href='javascript:history.back(1);'>Regresar</a><br>";
}
